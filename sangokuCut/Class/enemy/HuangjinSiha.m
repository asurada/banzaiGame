...
@implementation HuangjinSiha



+(id)spriteWithFile{
    return [super spriteWithSpriteFrameName:@"HuangjinSiha_normal_1.png"];
}
...
-(void)loadDeadAnim{
    self.deadSound = @"fingerHit_1.caf";
    NSMutableArray *deadAnimFrames = [NSMutableArray array];
    for (int i=1; i<=56; i++) {
        [deadAnimFrames addObject:
         [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:
          [NSString stringWithFormat:@"HuangjinSiha_dead_%d.png",i]]];
    }
...
-(void)loadNormalAnim{
    ...
    for (int i=1; i<=14; i++) {
        [normalAnimFrames addObject:
         [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:
          [NSString stringWithFormat:@"HuangjinSiha_normal_%d.png",i]]];
    }
